...
with open("/home/tajamul/MAT-Agent/data/gta_dataset/dataset.json", "r") as f:
    dataset = json.load(f)
...
if args.lora_path is None:
    if args.engine == "tonggpt":
        root = f"/share/data/drive_4/tj/MAT_Agent/results/gta/{config.tonggpt.model_name}"
    elif args.engine == "internvl2":
        root = f"/share/data/drive_4/tj/MAT_Agent/results/gta/{config.internvl2.model_name}"
    elif args.engine == "llava":
        root = f"/share/data/drive_4/tj/MAT_Agent/results/gta/{config.llava.model_name}"
    else:
        root = f"/share/data/drive_4/tj/MAT_Agent/results/gta/{config.qwen.model_name}"
else:
    if "checkpoint-" in args.lora_path:
        path = args.lora_path.strip('/').split("/")
        path = path[len(path)-2:]
        path = "_".join(path)
        root = f"/share/data/drive_4/tj/MAT_Agent/results/gta/{path}"
    else:
        root = os.path.join("/share/data/drive_4/tj/MAT_Agent/results/gta/", args.lora_path.strip("/").split("/")[-1])
