...
parser.add_argument(
    '--engine',
    '-e',
    choices=["minicpm", 'tonggpt', 'qwen','qwen25', "internvl2", "llava"],
    default="qwen"
)
...
class GAIAAgent():
    def __init__(self) -> None:
        os.makedirs(f"/share/data/drive_4/tj/MAT_Agent/results/gaia/{SPLIT}", exist_ok=True)
        os.makedirs(f"/share/data/drive_4/tj/MAT_Agent/results/gaia/{SPLIT}/{args.engine}", exist_ok=True)
        self.config = load_config()
        if args.engine in ["tonggpt"]:
            self.qa_cache_db = os.path.join("/share/data/drive_4/tj/MAT_Agent/results/gaia/", SPLIT, args.engine, self.config.tonggpt.model_name)
        elif args.engine in ["qwen"]:
            if args.lora_path is not None:
                self.qa_cache_db = os.path.join("/share/data/drive_4/tj/MAT_Agent/results/gaia/", SPLIT, args.engine, args.lora_path)
            else:
                self.qa_cache_db = os.path.join("/share/data/drive_4/tj/MAT_Agent/results/gaia/", SPLIT, args.engine, self.config.qwen.model_name)
        elif args.engine in ["qwen25"]:
            if args.lora_path is not None:
                self.qa_cache_db = os.path.join("/share/data/drive_4/tj/MAT_Agent/results/gaia/", SPLIT, args.engine, args.lora_path)
            else:
                self.qa_cache_db = os.path.join("/share/data/drive_4/tj/MAT_Agent/results/gaia/", SPLIT, args.engine, self.config.qwen25.model_name)
        elif args.engine in ["internvl2"]:
            if args.lora_path is not None:
                self.qa_cache_db = os.path.join("/share/data/drive_4/tj/MAT_Agent/results/gaia/", SPLIT, args.engine, args.lora_path)
            else:
                self.qa_cache_db = os.path.join("/share/data/drive_4/tj/MAT_Agent/results/gaia/", SPLIT, args.engine, self.config.internvl2.model_name)
        elif args.engine in ["minicpm"]:
            if args.lora_path is not None:
                self.qa_cache_db = os.path.join("/share/data/drive_4/tj/MAT_Agent/results/gaia/", SPLIT, args.engine, args.lora_path)
            else:
                self.qa_cache_db = os.path.join("/share/data/drive_4/tj/MAT_Agent/results/gaia/", SPLIT, args.engine, self.config.minicpm.model_name)
        elif args.engine in ["llava"]:
            self.qa_cache_db = os.path.join("/share/data/drive_4/tj/MAT_Agent/results/gaia/", SPLIT, args.engine, self.config.llava.model_name)
...
if __name__ == "__main__":
    ...
    if num_worker == 1:
        ...
        ds = load_dataset(
            ...
            split=SPLIT,
            token=os.environ.get("HF_TOKEN"),
            download_mode="force_redownload", verification_mode=VerificationMode.NO_CHECKS, trust_remote_code=True  # <- bypasses the format check
        )
        futures = []
