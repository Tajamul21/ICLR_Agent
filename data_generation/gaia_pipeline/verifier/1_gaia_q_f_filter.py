...
def main():
    ...
    for idx, item in tqdm(enumerate(data)):
        ...
        # build file_list from whatever schema the item has
        file_list = []

        # { "file": { "file_name": "x" or [...] } }
        if isinstance(item.get("file"), dict) and item["file"].get("file_name"):
            fn = item["file"]["file_name"]
            file_list = fn if isinstance(fn, list) else [fn]

        # top-level "file_name": "x" or [...]
        ...
        # "files": [ "x", {...}, ... ]
            ...
            for f in item["files"]:
                if isinstance(f, str):
                    ...
                    file_list.append(f.get("path") or f.get("file_name"))

        # "file_information": {"file_1": {"file_name": "..."} , ...}
        elif isinstance(item.get("file_information"), dict):
            for v in item["file_information"].values():
                if isinstance(v, dict) and v.get("file_name"):
                    file_list.append(v["file_name"])

        # make absolute under your save root
        root = Path(file_save_root_path).resolve()
        file_list = item.get("file", {}).get("file_name", [])
        if isinstance(file_list, str):
            file_list = [file_list]

        # make paths absolute under your save root, without double-prefixing
        file_list = [
            (Path(p).resolve() if Path(p).is_absolute()
            else (root / p).resolve())
            for p in file_list
        ]
